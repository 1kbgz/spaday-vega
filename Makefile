...
.PHONY: catalog
catalog:  ## regenerate the typed component from its manifest
	python -m spaday.cem spaday_vega/components.cem.json -o spaday_vega/components.py
	python -m ruff format spaday_vega/components.py
...
lint-py:  ## run python linter with ruff
	python -m ruff check spaday_vega
	python -m ruff format --check spaday_vega
...
fix-py:  ## fix python formatting with ruff
	python -m ruff check --fix spaday_vega
	python -m ruff format spaday_vega
...
test-py:  ## run python tests
	python -m pytest -v spaday_vega/tests

# alias
tests-py: test-py

coverage-py:  ## run python tests and collect test coverage
	python -m pytest -v spaday_vega/tests --cov=spaday_vega --cov-report term-missing --cov-report xml
...
pyodide-example: build  ## build the standalone Pyodide example into dist/lite
	test -n "$(firstword $(wildcard dist/spaday_vega-*.whl))"
	rm -rf dist/lite dist/pyodide-deps
	mkdir -p dist/lite dist/pyodide-deps
	python -m pip download --no-deps --only-binary=:all: --platform pyemscripten_2026_0_wasm32 --python-version 314 --implementation cp --abi cp314 --dest dist/pyodide-deps "spaday==0.8.2"
	python js/examples/build_pyodide_example.py dist/lite "$(firstword $(wildcard dist/spaday_vega-*.whl))" dist/pyodide-deps
...
test-pyodide-example: pyodide-example  ## run the standalone Pyodide example in Chromium
	...
	cd js; SPADAY_VEGA_PYODIDE_ONLY=1 pnpm exec playwright test tests/pyodide.spec.js
